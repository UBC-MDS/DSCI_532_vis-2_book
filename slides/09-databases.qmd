...
`code/lecture09/app-01-ibis-intro.py`
...
## file: app.py
{{< include /code/lecture09/app-02a-taxi-inmem.py >}}
...
## Why Local Breaks: The Pattern to Avoid

This works locally — but breaks on Posit Connect:

```python
LOG_FILE = Path("logs/query_log.csv")

def save_info(row: dict):
    df = pd.DataFrame([row])
...
def load_data() -> pd.DataFrame:
    ...
    return pd.DataFrame(columns=["timestamp", "user_query", "sql", "n_rows"])
```

. . .

::: {.callout-warning}
Works locally, though struggles with multiple users. On Posit Connect: file resets on restart, corrupts under multiple workers.
:::
